...
BEGIN
	...
	DECLARE gene_id BIGINT(20);
    DECLARE existing_gene_id BIGINT(20);
    ...
    DECLARE existing_strain_id BIGINT(20);
	# We check if the variant has already been canonicalized
START TRANSACTION;
	# 1. insert into variant_canon_identifier
	IF (SELECT EXISTS(SELECT * FROM variant_canon_identifier WHERE variant_ref_txt LIKE var_variant_ref_txt)) THEN
	...
	ELSE
		...
		SET var_canon_var_identifier_id = last_insert_id();
       # SELECT concat('variant canonicalized inserted with id: ', var_canon_var_identifier_id) ;
	END IF;
    
    # 2. insert into gene
     IF (SELECT EXISTS(SELECT * FROM gene WHERE name LIKE gene_name AND chromosome LIKE var_chr)) THEN
		SELECT id INTO existing_gene_id FROM gene WHERE name LIKE gene_name AND chromosome LIKE var_chr;
		SET gene_id = existing_gene_id;
		#SELECT concat('existing gene with id:', gene_id);
	ELSE
		INSERT INTO gene (version, chromosome, name) VALUES (0, var_chr, gene_name);
		SET gene_id = last_insert_id();
		#SELECT concat('new gene inserted with id:', gene_id);
	END IF;
	...
	INSERT INTO variant (version, chr, position, alt, ref, type, assembly, parent_ref_ind, parent_variant_ref_txt, variant_ref_txt, canon_var_identifier_id, gene_id)
        VALUES (0, var_chr, var_position, var_alt, var_ref, var_type, var_assembly, var_parent_ref_ind, var_parent_variant_ref_txt, var_variant_ref_txt, var_canon_var_identifier_id, gene_id);
   ...
   # SELECT concat('transcript inserted with id:', inserted_transcript_id);
    ...
    IF (SELECT EXISTS(SELECT * FROM strain WHERE name like strain_name)) THEN
		SELECT id INTO existing_strain_id FROM strain WHERE name like strain_name;
		SET strain_id = existing_strain_id;
	...
	INSERT INTO gene_identifier (gene_identifiers_id, identifier_id) VALUES (gene_id, inserted_identifier_id);
